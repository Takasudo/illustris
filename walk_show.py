...
# show results for saved quantities
'''
figname = run_num+"/fig_"+str(mag_label)+"_"+run_num+"_fpp_funcofE.pdf"  
fig = plt.figure(figsize=(7,9))
ax1 = fig.add_subplot(111)
mlist = ["X","D","*","8","s","P"]
llist = ["-","-.","--",":","-","--"]
for h in range (len(HaloID)):
    ax1.loglog(Epev,avg_fpp_Myr[h],color='black',label="ID:"+str(HaloID[h])+" (SFR :"+str(subhalo_sfr_list[h])+")",marker=mlist[h],ls=llist[h])
    ax1.loglog(Epev,avg_fpp_10Myr[h],color='blue',marker=mlist[h],ls=llist[h])
    ax1.loglog(Epev,avg_fpp_90Myr[h],color='red',marker=mlist[h],ls=llist[h])
ax1.set_xlabel("$E_p$ [PeV]",fontsize=18)
ax1.set_ylabel("<$f_{pp}$>",fontsize=18)
plt.legend()
plt.savefig(figname,bbox_inches='tight')
plt.close()
'''
